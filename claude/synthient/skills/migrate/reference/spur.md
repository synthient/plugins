...
Spur had no equivalent single score. Adopting it is optional, and Synthient's own guidance is that enterprises should score internally, since a 0–100 number collapses signals you may want to weigh separately. Do not restructure working per-signal logic around the score just because it is now available.
...
| Batch | n/a | `POST /api/v4/lookup/ips`, up to 1,000, `ceil(n*0.9)` credits |
| Rate limit | n/a | 100 req/sec sustained, per **team** not per key |
