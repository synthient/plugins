---
name: feeds
description: Work with Synthient bulk data — download and checksum-verify Parquet snapshots, capture filtered or long-running NDJSON streams, export CSV, and analyze the result locally. Covers the feed capabilities the MCP tools cannot reach, using the synthient CLI.
when_to_use: Triggers on "download the proxy feed", "get the Parquet snapshot", "capture the firehose", "stream proxies to a file", "analyze the torrent feed", "build a blocklist from the feed", or any bulk or long-running Synthient data work.
allowed-tools: Bash(synthient feeds:*) Bash(synthient download:*) Bash(synthient stream:*) Bash(synthient account:*)
---
...
The CLI needs to be installed: `brew install synthient/tap/synthient`, or `go install github.com/synthient/cli/cmd/synthient@latest`. It uses its own credential chain — `SYNTHIENT_API_KEY`, then `.env` in the working directory, then the OS keychain via `synthient auth`. Run `/synthient:doctor` if anything fails.
...
Always pass `--verify`. It checks the SHA-256 from the snapshot metadata against the downloaded bytes. The destination must end in `.parquet`. Presigned URLs expire in 24 hours and are minted per request — never cache or share one.

Feeds cost no lookup credits.
...
`stream` is always NDJSON — it has no `--format`. `--pretty` is for eyeballing only, never for piping. `--filter field=value` repeats, ANDs, and takes dot notation for nested fields, applied client-side. Bound the run with `--max-events` or `--duration`, and use `--reconnect` for anything long.
...
```bash
duckdb -c "SELECT * FROM './proxies.parquet' LIMIT 10"
duckdb -c "SELECT provider, count(*) FROM './proxies.parquet' GROUP BY 1 ORDER BY 2 DESC"
```

`feed_snapshot_meta` gives the column names and types before you download, which is usually enough to write the query up front.
