---
name: doctor
description: Diagnose a Synthient setup — whether the CLI is installed, which credential source is active, whether the account is reachable, which scopes are granted, how many lookup credits remain, and whether the MCP tools are connected. Use when a Synthient call fails or before relying on one.
