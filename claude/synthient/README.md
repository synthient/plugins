...
Then authenticate, either by entering your key when the plugin prompts on enable, or with the CLI:

```bash
synthient auth
```

Get a key from the dashboard at [synthient.com](https://synthient.com). Run `/synthient:doctor` to confirm everything resolved.
...
Claude also loads these on its own when the context calls for it, so you do not have to invoke them by name.
...
Also takes `ipqs`, or any other vendor name. It audits every call site with a read-only subagent, maps the fields, and **shows you the report before changing anything**.
