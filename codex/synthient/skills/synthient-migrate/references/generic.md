...
- **Fields with no equivalent.** Common gaps: browser and user-agent parsing, email or phone reputation, device fingerprinting, historical abuse counters. These need the logic rethought, not remapped. List them prominently — a migration that quietly drops a signal is worse than one that names the loss.
